...
// ─── Pairing Code (tanya nomor sekali di awal) ───────────────────────────────
async function askPhoneNumber() {
  return new Promise((resolve) => {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    rl.question('📱 Masukkan nomor WhatsApp (format: 628xxx): ', (number) => {
      rl.close();
      resolve(number.trim());
    });
  });
...
async function startBot() {
  ...
  const sock = makeWASocket({
    ...
    // Penting: naikkan batas listener agar tidak warning
    maxMsgRetryCount: 3
  });

  // Naikkan batas EventEmitter global agar tidak MaxListenersExceeded
  process.setMaxListeners(20);

  // ─── Pairing Code — hanya jika belum pernah login ──────────────────────────
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
    if (connection === 'open') {
      ...
      if (statusCode === DisconnectReason.loggedOut) {
        console.log('❌ Bot logout. Hapus folder auth_info lalu jalankan ulang.');
        process.exit(0);
      } else {
        // Delay sebelum reconnect agar tidak spam
        const delay = 5000;
        console.log(`🔄 Koneksi terputus (${statusCode}). Reconnect dalam ${delay / 1000}s...`);
